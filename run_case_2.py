...
def make_mesh(meshing,in_file,out_file,alpha_per,mode):

    time0 = time.time()

    #mesh size definitions
    max_size = 100 # mm
    min_size_blade = 0.5 # mm
    min_size_blade2 = 0.05 # mm
    min_size_duct_hub = 1 # mm
    curv_angle = 5 # deg
    cell_size_blade = 3 # mm
    cell_size_blade2 = 0.3 # mm
    cell_size_duct_hub = 6 # mm
...
def mesh(variables, const, case_folder):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        mode = 1
        future = executor.submit(make_mesh, meshing, geo_file, out_file, alpha_per, mode)
        try:
            mesh_file = future.result(timeout=600) # Wait for up to 10 minutes
        except Exception as e:
            mode = 2
            try:
                mesh_file = make_mesh(meshing, geo_file, out_file, alpha_per, mode)
            except Exception as e:
...
foutname = rf"Z:\UFX_dilhara\Ansys\3D_CFD_Optimizer\OptiSlang\projects\project3\case{case_id}\varients.csv"
...
for ivarient in range(nvarients):
    if os.path.exists(foutname):
        with open(foutname, "r") as file:
            nlines = 0
            for line in file:
                nlines += 1
        nlines -= 1  # Subtract the header line
        jvar = nlines + 1
        case_folder = rf"Z:\UFX_dilhara\Ansys\3D_CFD_Optimizer\OptiSlang\projects\project3\case{case_id}\varient{jvar}"
        print(f"Creating new case folder: {case_folder}")
    else:
        case_folder = rf"Z:\UFX_dilhara\Ansys\3D_CFD_Optimizer\OptiSlang\projects\project3\case{case_id}\varient1"

    #OSL_PROJECT_DIR = r"D:\Ansys\Optislang\Project_3\project3_3.opd"
    #case_input_file = rf"D:\Ansys\Optislang\Project_3\project3_3.opd\AMOP\Design{case_id:04d}\case_inuts.txt"
    case_input_file = rf"D:\Ansys\Optislang\Project_3\project3_3.opd\AMOP\Design{case_id:04d}\case_inuts.txt"
    ...
    try:
        ...
        print(f"Directory '{case_folder}' already exists.")
    #global_input_file = rf"{OSL_PROJECT_DIR}\inputs.yaml"
    global_input_file = rf"Z:\UFX_dilhara\Ansys\3D_CFD_Optimizer\OptiSlang\projects\project3\case1168\inputs.yaml"
    shutil.copyfile(global_input_file,rf"{case_folder}\inputs.yaml")
    ...
    if custom_inputs == 1: # custom mode
        ...
        print("Using case inputs")
        # Read file
        params = read_params(case_input_file)

        # --------- Rotor parameters ----------
        blade_count = params["Nb"]
        solidity = params["Solidity"]
        root_cutout = params["Rcout"]
        twist = params["Twist"]
        theta75 = params["Theta75"]
        AR_1 = params["AR_1"]
        AR_2 = params["AR_2"]
        AR_3 = params["AR_3"]
        ...
        air_thick_1 = params["Air_thick_1"]
        air_thick_2 = params["Air_thick_2"]
        air_thick_3 = params["Air_thick_3"]

        air_camber_1 = params["Air_camber_1"]
        air_camber_2 = params["Air_camber_2"]
        air_camber_3 = params["Air_camber_3"]

        # --------- Duct parameters ----------
        dlength = params["Duct_Length"]
        dwidth = params["Duct_Width"]
        din_exp = params["Duct_In_exp"]
        dout_exp = params["Duct_Out_exp"]

        # --------- Hub parameters ----------
        Hub_Length = params["Hub_Length"]

        # --------- Stator parameters ----------
        Stator_Nb = params["Stator_Nb"]
        Stator_Solidity = params["Stator_Solidity"]
        Stator_Taper = params["Stator_Taper"]
        Stator_Twist = params["Stator_Twist"]
        Stator_Theta75 = params["Stator_Theta75"]

        Stator_Air_thick_1 = params["Stator_Air_thick_1"]
        Stator_Air_thick_2 = params["Stator_Air_thick_2"]
        Stator_Air_thick_3 = params["Stator_Air_thick_3"]

        Stator_Air_camber_1 = params["Stator_Air_camber_1"]
        Stator_Air_camber_2 = params["Stator_Air_camber_2"]
        Stator_Air_camber_3 = params["Stator_Air_camber_3"]

        Stator_Rake = params["Stator_Rake"]
        Stator_Clearance = params["Stator_Clearance"]

    #print((f"{blade_count},{root_cutout},{solidity},{twist},{theta75},{air_thick_1},{air_thick_2},{air_thick_3},{air_camber_1},{air_camber_2},{air_camber_3},{AR_1},{AR_2},{AR_3},{SW_1},{SW_2},{SW_3},{ivarient//2}\n"))
    ...
    SW_2 = 0  
    SW_3 = 0 #-0.5 #-0.4
    #const[id.rair_loc[1]] = 0.5



    variables[id.rnb] = blade_count
    variables[id.rcout] = root_cutout
    ...
    #print((f"{blade_count},{root_cutout},{solidity},{twist},{theta75},{air_thick_1},{air_thick_2},{air_thick_3},{air_camber_1},{air_camber_2},{air_camber_3},{AR_1},{AR_2},{AR_3}\n"))
